...
// === TIERS ===
const tiers = [
  { name: "Basic", desc: "Para profesionales que quieren vivir el evento completo y acceder al contenido esencial.", old: "697 €", price: "497 €", includes: ["Acceso a los 2 días completos","Ponencias internacionales","Traducción simultánea portugués-español","Almuerzos día 1 y día 2","Networking","Acceso a la Cena Blanca de Gala con cena","Kit de bienvenida estándar","Certificado digital de participación","Vale de 497 € en formación"], excludes: ["Kit de negocio premium","Cena privada Divine Night","Alojamiento","Primera fila"], link: "https://buy.stripe.com/cNifZjdCn2gZ5WqgBH7kc00" },
  { name: "Gold", desc: "Para profesionales que quieren una experiencia más completa, con más valor y beneficios premium.", old: "997 €", price: "797 €", badge: "MÁS ELEGIDO", badgeClass: "tier-badge-gold", hl: true, includes: ["Todo lo incluido en Basic","Kit de negocio valorado en 200 €","Ubicación preferencial en la Cena Blanca de Gala","Regalo sorpresa del patrocinador","Certificado físico premium","Vale de 797 € en formación"], excludes: ["Cena privada Divine Night","Alojamiento incluido"], link: "https://buy.stripe.com/cNibJ30PB7BjacG7177kc01" },
  { name: "Divine VIP", desc: "Para quien quiere vivir el MAGColor Experience al máximo nivel. Solo 10 plazas.", old: "1.597 €", price: "1.297 €", badge: "SOLO 10 PLAZAS", badgeClass: "tier-badge-red", includes: ["Todo lo incluido en Gold","Primera fila garantida","Divine Night: cena privada VIP","2 noches de alojamiento (doble compartido)","Desayuno en el Hotel HF Fénix Lisboa","Acceso preferencial","Meet & Greet privado con ponentes","Derecho a voto popular en el campeonato","Credencial personalizada","Mesa VIP en la Cena Blanca de Gala","Vale de 1.297 € en formación"], excludes: [], link: "https://buy.stripe.com/6oUcN769V08RgB43OV7kc02" }
];
...
function renderTiers() {
  ...
  container.innerHTML = tiers.map(t => {
    ...
    return `<div class="tier${t.hl?' tier-hl':''}">${badge}<h3 class="tier-name">${t.name}</h3><p class="tier-desc">${t.desc}</p><div class="tier-price-old">${t.old}</div><div class="tier-price">${t.price}</div><div class="tier-tag">Early Bird</div><ul class="tier-list">${includes}</ul>${excludes}<a href="${t.link}" target="_blank" rel="noopener" class="btn btn-gold tier-btn">Comprar ${t.name}</a></div>`;
  }).join('');
}
